...
import uuid
import shutil
from MyPythonLib.Util import RequestUtil,JsonUtil
import hashlib
        ...
        if names[-1] == '/':
            ...
            if os.path.isfile(member):
                if isOverWrite:
                    ...
                    continue
            targetMkPath = getFilePath(member)
            if not os.path.isdir(targetMkPath):
                os.makedirs(targetMkPath)
...
def DeleDir(dirPath):
    try:
        ...
        pass
def downLoadFile(url,targetPath,showLog = True):
    ...
    RequestUtil.getFile(url,targetPath)
    # r = requests.get(url) 
